...
export default function ManusSidebar() {
  return (
    <aside
      ...
      style={{ background: '#1E1E20', borderRight: '1px solid #151517' }}
    >
      {/* Top bar */}
      ...
      <div className="px-2 flex flex-col gap-0.5">
        {['How to Prompt Changes f...', 'How to Code an Exact Replica of ...', 'SimilarWeb Analytics Overview'].map((title) => (
          <div
            key={title}
      ...
      <div className="mt-auto border-t border-[#2A2A2E] px-4 py-3">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-[13.5px] text-[#D0D0D4]">axuora.innovations</span>
        </div>
      </div>
    </aside>
  );
...
function NavItem({
  ...
  return (
    ...
    >
      ...
      {badge && (
        <span
          className="ml-auto text-[11px] px-1.5 py-0.5 rounded-md font-medium"
          style={{ background: '#0B2A4A', color: '#3AA0FF' }}
        >
          {badge}
        </span>
      )}
...
function SectionHeader({ label }: { label: string }) {
  return (
    <div className="mt-4 px-4 pb-1.5 text-[12.5px] text-[#8A8A91] flex items-center justify-between">
      <span>{label}</span>
      <span className="text-[#5F5F66] cursor-pointer">
        {label === 'Projects' ? <PlusIcon /> : <FilterIcon />}
      </span>
    </div>
  );
}
